...
describe("mapUtilityToFirmness", () => {
  it("returns 0.15 at the very-soft boundary (utility >= 0.80)", () => {
    expect(mapUtilityToFirmness(0.8)).toBe(0.15);
    expect(mapUtilityToFirmness(0.95)).toBe(0.15);
    expect(mapUtilityToFirmness(1.0)).toBe(0.15);
  });

  it("returns 0.35 in the comfortable zone (0.65–0.80)", () => {
    expect(mapUtilityToFirmness(0.65)).toBe(0.35);
    expect(mapUtilityToFirmness(0.7)).toBe(0.35);
    expect(mapUtilityToFirmness(0.79)).toBe(0.35);
  });

  it("returns 0.55 in the negotiation zone (0.50–0.65)", () => {
    expect(mapUtilityToFirmness(0.5)).toBe(0.55);
    expect(mapUtilityToFirmness(0.6)).toBe(0.55);
    expect(mapUtilityToFirmness(0.64)).toBe(0.55);
  });

  it("returns 0.75 in the escalation zone (0.35–0.50)", () => {
    expect(mapUtilityToFirmness(0.35)).toBe(0.75);
    expect(mapUtilityToFirmness(0.45)).toBe(0.75);
    expect(mapUtilityToFirmness(0.49)).toBe(0.75);
  });

  it("returns 0.90 in the walk-away zone (< 0.35)", () => {
    expect(mapUtilityToFirmness(0.34)).toBe(0.9);
    expect(mapUtilityToFirmness(0.1)).toBe(0.9);
    expect(mapUtilityToFirmness(0.0)).toBe(0.9);
  });
  ...
  it("handles boundary: 0.35 is escalation zone (not walk-away)", () => {
    expect(mapUtilityToFirmness(0.35)).toBe(0.75);
  });

  // Edge: out-of-range utility (defensive)
  it("handles utility > 1 gracefully (clamped to very-soft zone)", () => {
    expect(mapUtilityToFirmness(1.5)).toBe(0.15);
  });
...
describe("buildNegotiationIntent – action routing", () => {
  ...
  it("does NOT override action to MESO when mesoOffers is empty array", () => {
    const intent = buildNegotiationIntent(
      makeInput({ action: "COUNTER", mesoOffers: [] }),
    );
    expect(intent.action).toBe("COUNTER");
  });
  ...
  it("does NOT override action to MESO when mesoOffers is undefined", () => {
    const intent = buildNegotiationIntent(
      makeInput({ action: "COUNTER", mesoOffers: undefined }),
    );
    expect(intent.action).toBe("COUNTER");
  });
  ...
  it("returns ASK_CLARIFY when engine says ASK_CLARIFY", () => {
    const intent = buildNegotiationIntent(
      makeInput({ action: "ASK_CLARIFY", counterPrice: null }),
    );
...
describe("buildNegotiationIntent – firmness", () => {
  it("firmness is 0.35 for high-utility comfortable zone (0.65–0.80)", () => {
    const intent = buildNegotiationIntent(makeInput({ utilityScore: 0.75 }));
    expect(intent.firmness).toBe(0.35);
  });

  it("firmness is 0.15 for very-high utility (>=0.80, near acceptance)", () => {
    const intent = buildNegotiationIntent(makeInput({ utilityScore: 0.9 }));
    expect(intent.firmness).toBe(0.15);
  });

  it("firmness is 0.55 for mid-range utility", () => {
    const intent = buildNegotiationIntent(makeInput({ utilityScore: 0.6 }));
    expect(intent.firmness).toBe(0.55);
  });

  it("firmness is 0.75 for low utility (escalation zone)", () => {
    const intent = buildNegotiationIntent(makeInput({ utilityScore: 0.4 }));
...
describe("buildNegotiationIntent – price boundary guard (COUNTER)", () => {
  ...
  it("returns undefined allowedPrice when counterPrice is undefined", () => {
    const intent = buildNegotiationIntent(
      makeInput({ counterPrice: undefined }),
    );
    expect(intent.allowedPrice).toBeUndefined();
  });
...
describe("buildNegotiationIntent – ACCEPT/WALK_AWAY/ESCALATE have no pricing fields", () => {
  const noPrice: Array<BuildIntentInput["action"]> = [
    "ACCEPT",
    "WALK_AWAY",
    "ESCALATE",
  ];
...
describe("buildNegotiationIntent – COUNTER optional fields", () => {
  it("includes allowedPaymentTerms when provided", () => {
    const intent = buildNegotiationIntent(
      makeInput({ counterPaymentTerms: "Net 60" }),
    );
  ...
  it("omits allowedPaymentTerms when not provided", () => {
    const intent = buildNegotiationIntent(
      makeInput({ counterPaymentTerms: null }),
    );
    expect(intent.allowedPaymentTerms).toBeUndefined();
  });
  ...
  it("includes allowedDelivery when provided", () => {
    const intent = buildNegotiationIntent(
      makeInput({ counterDelivery: "45 days" }),
    );
...
describe("buildNegotiationIntent – commercialPosition", () => {
  const actions: BuildIntentInput["action"][] = [
    "ACCEPT",
    "COUNTER",
    "WALK_AWAY",
    "ESCALATE",
  ...
  it("selects friendly ACCEPT position when tone is friendly", () => {
    const intent = buildNegotiationIntent(
      makeInput({ action: "ACCEPT", tone: "friendly" }),
    );
...
describe("Persona: experienced vendor — precision counter-offer", () => {
  it("handles very precise counter prices (decimals)", () => {
    const intent = buildNegotiationIntent(
      makeInput({
        counterPrice: 98750.5,
        targetPrice: 95000,
        maxAcceptablePrice: 100000,
      }),
    );
    expect(intent.allowedPrice).toBe(99000);
  });
