...
describe("AI Eval: ValidationAgent", () => {
  it("should pass valid output and clean soft phrases", async () => {
    ...
    const rawContent = "I am here to help! We can accept the offer at $10,000.";
    const mockState = {
      decision: { action: "ACCEPT" },
      config: { currency: "USD" },
      parsedOffer: { totalPrice: 10000 },
    ...
    expect(result.messages?.[0].content).toContain("We can accept the offer at $10,000.");
  ...
  it("should fail when price is unauthorized in COUNTER", async () => {
    ...
    const rawContent = "Our counter offer is $8,500 for the total package.";
    const mockState = {
      decision: { action: "COUNTER" },
      config: { currency: "USD" },
