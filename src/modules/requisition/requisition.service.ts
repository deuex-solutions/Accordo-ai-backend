...
import type { RequisitionData } from "./requisition.repo.js";
...
export const createRequisionService = async (
  ...
  try {
    logger.info("=== CREATE REQUISITION SERVICE ===");
    logger.info("Received requisitionData keys:", Object.keys(requisitionData));
    logger.info(
    ...
    logger.info("User found with companyId:", user.companyId);
    ...
    try {
      ...
      logger.info("Requisition created successfully with id:", requisition.id);
...
export const updateRequisitionService = async (
  ...
  try {
    ...
    logger.info("updateRequisitionService called with:", {
      requisitionId,
      userId,
    ...
    logger.info("Cleaned data for update:", cleanedData);
    logger.info("Product data parsed:", productData);

    let result;
    try {
      result = await repo.updateRequisition(requisitionId, cleanedData);
      logger.info("Requisition update result:", result);
    ...
    if (validProducts.length > 0) {
      ...
      logger.info("Deleting existing products for requisition:", requisitionId);
      await repo.deleteRequisitionProducts(requisitionId);

      logger.info("Creating new products:", validProducts.length);
