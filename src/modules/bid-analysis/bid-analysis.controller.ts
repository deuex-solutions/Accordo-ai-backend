...
import {
  ...
  selectBidBodySchema,
  rejectBidBodySchema,
} from "./bid-analysis.validator.js";
...
export async function getRequisitions(
  ...
  try {
    ...
    const result = await getRequisitionsForBidAnalysis(
      userId,
      companyId,
      value,
    );
  ...
  try {
    ...
    const companyId =
      req.context?.userType === "super_admin"
        ? null
        : req.context?.companyId || null;
    const result = await getRequisitionBidDetail(
      value.requisitionId,
      companyId,
    );
...
export async function selectBid(
  ...
  try {
    const { error: paramsError, value: params } = bidIdParamSchema.validate(
      req.params,
    );
    if (paramsError) {
    ...
    const { error: bodyError, value: body } = selectBidBodySchema.validate(
      req.body,
    );
    if (bodyError) {
...
export async function rejectBidHandler(
  ...
  try {
    ...
    const { error: bodyError, value: body } = rejectBidBodySchema.validate(
      req.body,
    );
    if (bodyError) {
      throw new CustomError(bodyError.details[0].message, 400);
...
export async function downloadPdfHandler(
  ...
  try {
    ...
    const hasSelectedVendor = !!(
      detail.selectedBidId && detail.selectedVendorId
    );
    ...
    const drawHeader = () => {
      ...
      doc
        ...
        .text("ACCORDO", margin, 20, { lineBreak: false });
      doc
        .fontSize(10)
        .fillColor(colors.primaryLight)
        .text(" AI", margin + 138, 30, { lineBreak: false });

      doc
        .fontSize(16)
        .fillColor(colors.white)
        .text("Bid Comparison Report", margin, 45, { lineBreak: false });
    ...
    const drawFooter = (pageNum: number, total: number) => {
      ...
      doc.text("Powered by Accordo AI", pageWidth / 2 - 50, footerY, {
        width: 100,
        align: "center",
        lineBreak: false,
      });
    ...
    const drawSectionHeader = (title: string, y: number) => {
      ...
      doc
        ...
        .text(title.toUpperCase(), margin + 15, y + 8, { lineBreak: false });
      doc.y = savedY;
      return y + 35;
    };
    ...
    drawInfoCard(
      ...
      currentY,
      "Project",
      detail.requisition.projectName || "N/A",
      colors.primary,
    );
    drawInfoCard(
      ...
      currentY,
      "Category",
      detail.requisition.category || "N/A",
      colors.primary,
    );
    drawInfoCard(
      ...
      currentY,
      "Deadline",
      detail.requisition.negotiationClosureDate
        ? new Date(
            detail.requisition.negotiationClosureDate,
          ).toLocaleDateString()
        : "N/A",
    ...
    drawStatBox(
      ...
      currentY,
      "Total Vendors",
      String(detail.requisition.totalVendors),
      "",
      colors.primary,
    );
    drawStatBox(
      margin + statBoxWidth + 10,
      currentY,
      "Completed",
      String(detail.requisition.completedVendors),
      `${Math.round((detail.requisition.completedVendors / detail.requisition.totalVendors) * 100) || 0}%`,
      colors.success,
    );
    drawStatBox(
      margin + (statBoxWidth + 10) * 2,
      currentY,
      "Lowest Bid",
      detail.priceRange.lowest
        ? `$${detail.priceRange.lowest.toLocaleString()}`
        : "N/A",
      "",
      colors.successLight,
    );
    drawStatBox(
      margin + (statBoxWidth + 10) * 3,
      currentY,
      "Highest Bid",
      detail.priceRange.highest
        ? `$${detail.priceRange.highest.toLocaleString()}`
        : "N/A",
      "",
      colors.warning,
    );
    drawStatBox(
      margin + (statBoxWidth + 10) * 4,
      currentY,
      "Average",
      detail.priceRange.average
        ? `$${Math.round(detail.priceRange.average).toLocaleString()}`
        : "N/A",
      "",
      colors.textLight,
    );
    ...
    detail.topBids.slice(0, 3).forEach((bid: TopBidInfo, index: number) => {
      ...
      doc
        .fontSize(10)
        .fillColor(colors.white)
        .text(rankLabels[index], x + 10, currentY + 7, { lineBreak: false });

      if (bid.isRejected) {
        doc
          .rect(x + topBidWidth - 60, currentY + 4, 50, 17)
          .fill(colors.danger);
        doc
          .fontSize(8)
          .fillColor(colors.white)
          .text("REJECTED", x + topBidWidth - 55, currentY + 8, {
            lineBreak: false,
          });
      }
      ...
      doc
        .fontSize(11)
        .fillColor(colors.text)
        .text(bid.vendorName, x + 10, currentY + 35, {
          width: topBidWidth - 20,
          ellipsis: true,
          lineBreak: false,
        });
      doc
        .fontSize(9)
        .fillColor(colors.textLight)
        .text(bid.vendorEmail, x + 10, currentY + 50, {
          width: topBidWidth - 20,
          ellipsis: true,
          lineBreak: false,
        });
      ...
      doc
        ...
        .fillColor(rankColor)
        .text(
          `$${bid.finalPrice?.toLocaleString() || "N/A"}`,
          x + 10,
          currentY + 70,
          { lineBreak: false },
        );
      ...
      doc.text(
        `Utility: ${bid.utilityScore ? `${(bid.utilityScore * 100).toFixed(1)}%` : "N/A"}`,
        x + topBidWidth / 2,
        currentY + 95,
        { lineBreak: false },
      );
    ...
    detail.allBids.forEach((bid, index) => {
      ...
      doc
        .fontSize(9)
        .fillColor(bid.isRejected ? colors.textLight : colors.text);

      const statusText = isSelected
        ? "SELECTED"
        : bid.isRejected
          ? "Rejected"
          : bid.dealStatus || "Pending";
    ...
    if (hasSelectedVendor) {
      ...
      const selectedBid = detail.allBids.find(
        (b) => b.bidId === detail.selectedBidId,
      );
      ...
      doc
        ...
        .text("VENDOR SELECTED", margin + 20, currentY + 10, {
          lineBreak: false,
        });
      doc
        .fontSize(18)
        .fillColor(colors.text)
        .text(detail.selectedVendorName || "N/A", margin + 20, currentY + 28, {
          lineBreak: false,
        });

      doc
        .rect(pageWidth - margin - 100, currentY + 15, 90, 25)
        .fill(colors.success);
      doc
        .fontSize(10)
        .fillColor(colors.white)
        .text("AWARDED", pageWidth - margin - 95, currentY + 22, {
          lineBreak: false,
        });
      ...
      if (selectedBid) {
        ...
        const drawDetailRow = (
          ...
          doc
            .fontSize(9)
            .fillColor(colors.textLight)
            .text(label, dx + 10, dy + 5, { lineBreak: false });
          doc
            .fontSize(11)
            .fillColor(colors.text)
            .text(val, dx + 10, dy + 17, { lineBreak: false });
          doc.y = savedDY;
        };
        ...
        drawDetailRow(
          ...
          "Email",
          selectedBid.vendorEmail,
        );
        drawDetailRow(
          leftColX,
          currentY + detailRowHeight * 2,
          "Final Price",
          selectedBid.finalPrice
            ? `$${selectedBid.finalPrice.toLocaleString()}`
            : "N/A",
          true,
        );
        drawDetailRow(
          leftColX,
          currentY + detailRowHeight * 3,
          "Unit Price",
          selectedBid.unitPrice
            ? `$${selectedBid.unitPrice.toLocaleString()}`
            : "N/A",
        );
        ...
        drawDetailRow(
          ...
          currentY + detailRowHeight * 2,
          "Utility Score",
          selectedBid.utilityScore
            ? `${(selectedBid.utilityScore * 100).toFixed(1)}%`
            : "N/A",
          true,
        );
        drawDetailRow(
        ...
        if (detail.priceRange.highest && selectedBid.finalPrice) {
          ...
          const savingsPercent = (
            (savings / detail.priceRange.highest) *
            100
          ).toFixed(1);
          const vsAverage = detail.priceRange.average
            ? detail.priceRange.average - selectedBid.finalPrice
            : null;
          const vsTarget = detail.priceRange.targetPrice
            ? detail.priceRange.targetPrice - selectedBid.finalPrice
            : null;

          const analysisBoxWidth = (contentWidth - 30) / 4;
          const analysisBoxHeight = 70;

          const drawAnalysisBox = (
            ax: number,
            ay: number,
            label: string,
            val: string,
            ...
            const savedAY = doc.y;
            doc
              .rect(ax, ay, analysisBoxWidth, analysisBoxHeight)
              .fill(positive ? "#dcfce7" : "#fef2f2");
            doc
              .rect(ax, ay, analysisBoxWidth, analysisBoxHeight)
              .stroke(positive ? "#86efac" : "#fecaca");
            doc
              .fontSize(18)
              .fillColor(positive ? colors.success : colors.danger)
              .text(val, ax, ay + 15, {
          ...
          if (vsAverage !== null) {
            drawAnalysisBox(
              margin + analysisBoxWidth + 10,
              currentY,
              "vs Average",
              `$${Math.round(vsAverage).toLocaleString()}`,
              vsAverage >= 0 ? "Below average" : "Above average",
              vsAverage >= 0,
            );
          }
          if (vsTarget !== null) {
            drawAnalysisBox(
              margin + (analysisBoxWidth + 10) * 2,
              currentY,
              "vs Target",
              `$${Math.round(vsTarget).toLocaleString()}`,
              vsTarget >= 0 ? "Under target" : "Over target",
              vsTarget >= 0,
            );
          }
          drawAnalysisBox(
            margin + (analysisBoxWidth + 10) * 3,
            currentY,
            "Award Price",
            `$${selectedBid.finalPrice.toLocaleString()}`,
            "Final negotiated",
    ...
    if (hasNegotiationData) {
      ...
      const drawNegTableHeader = () => {
        const savedY = doc.y;
        doc
          .rect(margin, currentY, contentWidth, negRowHeight)
          .fill(colors.primary);
        let tx = margin + 5;
        doc.fontSize(8).fillColor(colors.white);
        negHeaders.forEach((header, i) => {
          doc.text(header, tx, currentY + 10, {
            width: negColWidths[i] - 10,
            lineBreak: false,
          });
          tx += negColWidths[i];
        });
      ...
      negotiationHistory.forEach(
        (vendor: VendorNegotiationSummary, index: number) => {
          if (currentY > pageHeight - 60) {
            drawFooter(pageNumber, totalPages);
            doc.addPage();
            pageNumber++;
          ...
          doc.text(vendor.vendorName, x, currentY + 5, {
            width: negColWidths[0] - 10,
            ellipsis: true,
            lineBreak: false,
          });
          doc.fontSize(6).fillColor(colors.textLight);
          doc.text(vendor.vendorEmail, x, currentY + 17, {
            width: negColWidths[0] - 10,
            ellipsis: true,
            lineBreak: false,
          });
          ...
          doc
            .fontSize(8)
            .fillColor(statusColors[vendor.dealStatus] || colors.text);
          doc.text(vendor.dealStatus, x, currentY + 10, {
            width: negColWidths[1] - 10,
            lineBreak: false,
          });
          x += negColWidths[1];
          ...
          // Mode
          doc.fontSize(8).fillColor(colors.text);
          doc.text(vendor.mode, x, currentY + 10, {
            width: negColWidths[2] - 10,
            lineBreak: false,
          });
          x += negColWidths[2];
          ...
          const journey =
            vendor.startingPrice > 0
              ...
              : vendor.finalPrice > 0
                ? `$${vendor.finalPrice.toLocaleString()}`
                : "N/A";
          doc
            .fontSize(8)
            .fillColor(
              vendor.priceReductionPercent > 0 ? colors.success : colors.text,
            );
          doc.text(journey, x, currentY + 10, {
            width: negColWidths[3] - 10,
            ellipsis: true,
            lineBreak: false,
          });
          x += negColWidths[3];
          ...
          doc.text(
            ...
            x,
            currentY + 10,
            { width: negColWidths[4] - 10, lineBreak: false },
          );
          x += negColWidths[4];
          ...
          doc.fontSize(8).fillColor(colors.text);
          doc.text(
            vendor.utilityScore != null
              ? `${(vendor.utilityScore * 100).toFixed(1)}%`
              : "N/A",
            x,
            currentY + 10,
            { width: negColWidths[5] - 10, lineBreak: false },
          );
          x += negColWidths[5];

          // Payment Terms
          doc.fontSize(8).fillColor(colors.text);
          doc.text(vendor.paymentTerms || "N/A", x, currentY + 10, {
            width: negColWidths[6] - 10,
            ellipsis: true,
            lineBreak: false,
          });
      ...
      const sortedByUtility = [...negotiationHistory].sort(
        (a, b) => (b.utilityScore ?? 0) - (a.utilityScore ?? 0),
      );
      const top3Vendors = sortedByUtility.slice(0, 3);
      const otherVendors = sortedByUtility.slice(3);

      currentY = drawSectionHeader(
        "Detailed Price History — Top 3 Vendors",
        currentY,
      );
      ...
      for (const vendor of top3Vendors) {
        ...
        doc
          .rect(margin, currentY, contentWidth / 2 + 40, phRowHeight)
          .fill(colors.primaryDark);
        let tx = margin + 5;
        doc.fontSize(8).fillColor(colors.white);
        phHeaders.forEach((h, i) => {
          doc.text(h, tx, currentY + 6, {
            width: phColWidths[i] - 10,
            lineBreak: false,
          });
          tx += phColWidths[i];
        });
        ...
        if (rounds.size === 0) {
          ...
          doc
            .fontSize(8)
            .fillColor(colors.textLight)
            .text("No rounds completed", margin + 10, currentY + 5, {
              lineBreak: false,
            });
          doc.y = savedNRY;
          currentY += phRowHeight;
          ...
          for (const [roundNum, msgs] of rounds) {
            ...
            tx = margin + 5;
            doc.fontSize(8).fillColor(colors.text);
            doc.text(`R${roundNum}`, tx, currentY + 6, {
              width: phColWidths[0] - 10,
              lineBreak: false,
            });
            tx += phColWidths[0];

            doc.text(
              vendorMsg && vendorMsg.price > 0
                ? `$${vendorMsg.price.toLocaleString()}`
                : "-",
              tx,
              currentY + 6,
              { width: phColWidths[1] - 10, lineBreak: false },
            );
            tx += phColWidths[1];

            doc.fontSize(7).fillColor(colors.textLight);
            doc.text(accordoMsg?.decisionAction || "-", tx, currentY + 6, {
              width: phColWidths[2] - 10,
              ellipsis: true,
              lineBreak: false,
            });
            tx += phColWidths[2];
            ...
            doc.fontSize(8).fillColor(colors.text);
            doc.text(
              utility != null ? `${(utility * 100).toFixed(1)}%` : "-",
              tx,
              currentY + 6,
              { width: phColWidths[3] - 10, lineBreak: false },
            );
      ...
      drawRecBox(
        ...
        bestUtilityVendor?.utilityScore != null
          ? `Utility: ${(bestUtilityVendor.utilityScore * 100).toFixed(1)}%`
          : "",
        colors.success,
      );
      ...
      // Risk Flags
      const walkedAway = negotiationHistory.filter(
        (v) => v.dealStatus === "WALKED_AWAY",
      ).length;
      const lowUtility = negotiationHistory.filter(
        (v) => v.utilityScore != null && v.utilityScore < 0.3,
      ).length;
      drawRecBox(
        margin + (recBoxWidth + 10) * 2,
        currentY,
        "Risk Flags",
      ...
      const targetDiff =
        detail.priceRange.targetPrice && lowestPrice > 0
          ? detail.priceRange.targetPrice - lowestPrice
          : null;
      drawRecBox(
        margin + (recBoxWidth + 10) * 3,
        currentY,
        "Savings Potential",
        spread > 0 ? `$${spread.toLocaleString()}` : "N/A",
        targetDiff != null
          ? `${targetDiff >= 0 ? "$" + Math.round(targetDiff).toLocaleString() + " under" : "$" + Math.round(Math.abs(targetDiff)).toLocaleString() + " over"} target`
          : `Spread across ${negotiationHistory.length} vendors`,
        colors.warning,
      );
      ...
      try {
        const vendorDataForLlm = negotiationHistory.map((v) => ({
          ...
          utility:
            v.utilityScore != null
              ? `${(v.utilityScore * 100).toFixed(1)}%`
              : "N/A",
        }));

        const llmPromise = generateChatCompletion(
          [
            {
              role: "system",
              content:
                "You are a procurement analyst. Summarize the following vendor negotiation data in 1-2 concise paragraphs. Highlight the best value vendor, key risks, and a recommended next step. Do not use markdown formatting.",
            },
            {
              role: "user",
      ...
      if (narrative) {
        ...
        doc
          .fontSize(9)
          .fillColor(colors.text)
          .text(narrative, margin + 5, currentY, {
