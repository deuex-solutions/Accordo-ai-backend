  ...
  return {
    ...
    priceReductionPercent:
      priceReductionPercent !== null
        ? Number(priceReductionPercent.toFixed(2))
        : null,
    initialPaymentTerms,
    finalPaymentTerms,
    keyDecisions: keyDecisions.slice(-5), // Keep last 5 decisions
    negotiationDurationHours:
      negotiationDurationHours !== null
        ? Number(negotiationDurationHours.toFixed(2))
        : null,
    averageUtilityScore:
      averageUtilityScore !== null
        ? Number(averageUtilityScore.toFixed(4))
        : null,
  };
}
...
function generateFallbackSummary(
  ...
  const vendorMessages = messages.filter((m) => m.role === "VENDOR");
  const lastVendorOffer =
    vendorMessages[vendorMessages.length - 1]?.extractedOffer;
