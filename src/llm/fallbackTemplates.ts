...
import type { NegotiationIntent, VendorTone } from '../negotiation/intent/buildNegotiationIntent.js';
...
const COUNTER_TEMPLATES: Record<VendorTone | 'default', TemplatePool> = {
  formal: [
    (i) => `Thank you for your proposal. After careful review, we would like to respectfully counter with a total price of $${i.allowedPrice?.toLocaleString()}${i.allowedPaymentTerms ? `, with ${i.allowedPaymentTerms} payment terms` : ''}${i.allowedDelivery ? `, and delivery ${i.allowedDelivery}` : ''}. We believe these terms represent a fair path forward.`,
    (i) => `We appreciate the offer and wish to propose the following counter: $${i.allowedPrice?.toLocaleString()} total${i.allowedPaymentTerms ? ` on ${i.allowedPaymentTerms}` : ''}${i.allowedDelivery ? `, delivery ${i.allowedDelivery}` : ''}. We trust this aligns with both parties' objectives.`,
    (i) => `Following our review, we propose $${i.allowedPrice?.toLocaleString()}${i.allowedPaymentTerms ? ` with ${i.allowedPaymentTerms}` : ''}${i.allowedDelivery ? ` and delivery by ${i.allowedDelivery}` : ''} as our counter. We remain open to discussion.`,
  ],
  casual: [
    (i) => `Thanks for coming back to us! Here's what we can work with: $${i.allowedPrice?.toLocaleString()} total${i.allowedPaymentTerms ? `, ${i.allowedPaymentTerms}` : ''}${i.allowedDelivery ? `, delivery ${i.allowedDelivery}` : ''}. Think we can make that work?`,
    (i) => `Appreciate the offer — our counter is $${i.allowedPrice?.toLocaleString()}${i.allowedPaymentTerms ? ` on ${i.allowedPaymentTerms}` : ''}${i.allowedDelivery ? `, and delivery ${i.allowedDelivery}` : ''}. Let us know what you think!`,
    (i) => `Good to hear from you. We'd like to propose $${i.allowedPrice?.toLocaleString()}${i.allowedPaymentTerms ? ` with ${i.allowedPaymentTerms}` : ''}${i.allowedDelivery ? `, delivery ${i.allowedDelivery}` : ''}. Does that work for you?`,
  ],
  urgent: [
    (i) => `Given our timeline, we need to move quickly. Our counter is $${i.allowedPrice?.toLocaleString()}${i.allowedPaymentTerms ? `, ${i.allowedPaymentTerms}` : ''}${i.allowedDelivery ? `, delivery ${i.allowedDelivery}` : ''}. Can we confirm this today?`,
    (i) => `To keep things on track, here's our counter: $${i.allowedPrice?.toLocaleString()}${i.allowedPaymentTerms ? ` / ${i.allowedPaymentTerms}` : ''}${i.allowedDelivery ? ` / delivery ${i.allowedDelivery}` : ''}. We'd appreciate a fast turnaround.`,
    (i) => `We're working against a deadline — our offer is $${i.allowedPrice?.toLocaleString()}${i.allowedPaymentTerms ? ` with ${i.allowedPaymentTerms}` : ''}${i.allowedDelivery ? `, delivery ${i.allowedDelivery}` : ''}. Please let us know as soon as possible.`,
  ],
  firm: [
    (i) => `We have reviewed the offer and our counter stands at $${i.allowedPrice?.toLocaleString()}${i.allowedPaymentTerms ? ` with ${i.allowedPaymentTerms}` : ''}${i.allowedDelivery ? `, delivery ${i.allowedDelivery}` : ''}. This reflects our firm position given current constraints.`,
    (i) => `Our counter is $${i.allowedPrice?.toLocaleString()}${i.allowedPaymentTerms ? `, ${i.allowedPaymentTerms}` : ''}${i.allowedDelivery ? `, ${i.allowedDelivery} delivery` : ''}. We have limited flexibility beyond this.`,
    (i) => `After internal review, our position is $${i.allowedPrice?.toLocaleString()}${i.allowedPaymentTerms ? ` / ${i.allowedPaymentTerms}` : ''}${i.allowedDelivery ? ` / delivery ${i.allowedDelivery}` : ''}. We hope we can reach agreement on these terms.`,
  ],
  friendly: [
    (i) => `Really appreciate your proposal! We'd love to meet somewhere in the middle — our counter is $${i.allowedPrice?.toLocaleString()}${i.allowedPaymentTerms ? ` with ${i.allowedPaymentTerms}` : ''}${i.allowedDelivery ? `, delivery ${i.allowedDelivery}` : ''}. What do you think?`,
    (i) => `Thanks so much for the offer — we're making progress! Our counter: $${i.allowedPrice?.toLocaleString()}${i.allowedPaymentTerms ? `, ${i.allowedPaymentTerms}` : ''}${i.allowedDelivery ? `, ${i.allowedDelivery} delivery` : ''}. Hoping we can find the right fit.`,
    (i) => `We value this partnership and want to find terms that work for everyone. Our counter is $${i.allowedPrice?.toLocaleString()}${i.allowedPaymentTerms ? ` on ${i.allowedPaymentTerms}` : ''}${i.allowedDelivery ? `, delivery ${i.allowedDelivery}` : ''}. Happy to discuss further!`,
  ],
  default: [
    (i) => `Thank you for your offer. Our counter is $${i.allowedPrice?.toLocaleString()} total${i.allowedPaymentTerms ? `, ${i.allowedPaymentTerms}` : ''}${i.allowedDelivery ? `, delivery ${i.allowedDelivery}` : ''}. We believe this is a fair step forward.`,
    (i) => `We appreciate your proposal. After consideration, we're countering with $${i.allowedPrice?.toLocaleString()}${i.allowedPaymentTerms ? ` / ${i.allowedPaymentTerms}` : ''}${i.allowedDelivery ? ` / ${i.allowedDelivery}` : ''}. Can we find common ground here?`,
    (i) => `I appreciate your offer and want to keep this moving. Our counter: $${i.allowedPrice?.toLocaleString()}${i.allowedPaymentTerms ? ` with ${i.allowedPaymentTerms}` : ''}${i.allowedDelivery ? `, delivery ${i.allowedDelivery}` : ''}. Open to your thoughts.`,
    (i) => `Thanks for the proposal — here's our counter: $${i.allowedPrice?.toLocaleString()}${i.allowedPaymentTerms ? `, ${i.allowedPaymentTerms} terms` : ''}${i.allowedDelivery ? `, ${i.allowedDelivery} delivery` : ''}. Let's see if we can land on this.`,
    (i) => `We've reviewed your offer carefully. Our counter position is $${i.allowedPrice?.toLocaleString()}${i.allowedPaymentTerms ? ` on ${i.allowedPaymentTerms}` : ''}${i.allowedDelivery ? ` with delivery ${i.allowedDelivery}` : ''}. Looking forward to your response.`,
  ],
};
